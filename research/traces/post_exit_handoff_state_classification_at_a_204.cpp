...
namespace
{
    ...
    std::atomic<bool> g_holdAForClassification{false};
    std::atomic<bool> g_holdAConsumed{false};
    ...
    {
        g_holdAForClassification.store(false, std::memory_order_release);
        g_holdAConsumed.store(false, std::memory_order_release);
        bool expected = false;
    ...
    {
        if (g_holdAForClassification.load(std::memory_order_acquire)) {
            if (!g_holdAConsumed.exchange(true, std::memory_order_acq_rel))
                Log("DIAGNOSTIC consume post-exit replay: consumeCount=1 B_CONSTRAIN executed=false C_RESTORE executed=false cameraWrites=0.");
            return;
        }
        ...
        if (state == CoordinatorState::CinematicExiting) {
            ...
            if (flags == 0x04 && std::isfinite(currentFov) && std::isfinite(targetFov) && delta <= kRecoveryEpsilon) {
                ...
                g_holdAForClassification.store(true, std::memory_order_release);
                g_holdAConsumed.store(false, std::memory_order_release);
    ...
    DWORD WINAPI Initialize(void*)
    {
        ...
        const auto logPath = std::filesystem::path(modulePath).remove_filename() / "STALKER2PostExitHandoffStateClassificationAtA204.log";
        std::ofstream(logPath, std::ios::out | std::ios::trunc).close();
        try {
            g_logger = spdlog::basic_logger_mt("STALKER2PostExitHandoffStateClassificationAtA204", logPath.string(), true);
