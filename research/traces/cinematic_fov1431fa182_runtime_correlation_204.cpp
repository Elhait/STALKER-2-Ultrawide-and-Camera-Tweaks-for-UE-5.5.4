...
namespace
{
    ...
    constexpr std::uintptr_t kCandidateRva = 0x31FA182;
    ...
    constexpr std::uint8_t kCandidateBytes[] = {
        0x56, 0x48, 0x83, 0xEC, 0x40, 0x0F, 0x29, 0x7C, 0x24, 0x30,
    };
    ...
    bool ReadSetupState(std::uintptr_t object, float& s0, float& s1, float& s2,
        float& s3, float& s4, float& s5, float& s6, float& s7)
    {
        return SafeRead(object + 0x00, s0) && SafeRead(object + 0x04, s1) &&
            SafeRead(object + 0x08, s2) && SafeRead(object + 0x0C, s3) &&
            SafeRead(object + 0x10, s4) && SafeRead(object + 0x14, s5) &&
            SafeRead(object + 0x18, s6) && SafeRead(object + 0x1C, s7);
    }
    ...
    void Candidate(SafetyHookContext& context)
    {
        ...
        float s6{};
        float s7{};
        if (!object || !ReadSetupState(object, s0, s1, s2, s3, s4, s5, s6, s7))
            return;
        ...
        if (g_logger)
            g_logger->info(
                "TRACE seq={} stage=candidate-hit candidate=FUN_1431FA182 siteRva=0x{:X} "
                "object=0x{:X} inner=0x{:X} fov={} aspect={} flags=0x{:02X} "
                "argRdx=0x{:X} argR8=0x{:X} argR9=0x{:X} xmm1={} xmm3={} "
                "s0={} s1={} s2={} s3={} s4={} s5={} s6={} s7={} dtMs={} "
                ...
                static_cast<std::uintptr_t>(context.r9), context.xmm1.f32[0], context.xmm3.f32[0],
                s0, s1, s2, s3, s4, s5, s6, s7, dtMs, thread,
    ...
    DWORD WINAPI Initialize(void*)
    {
        ...
        const auto logPath = std::filesystem::path(modulePath).remove_filename() /
            "STALKER2CinematicFov1431FA182RuntimeCorrelation204.log";
        std::ofstream(logPath, std::ios::trunc).close();

        try {
            g_logger = spdlog::basic_logger_mt(
                "STALKER2CinematicFov1431FA182RuntimeCorrelation204", logPath.string(), true);
            ...
            g_logger->info(
                "TRACE installed: 2.0.4 FUN_1431FA182 runtime correlation; read-only, no writes.");
