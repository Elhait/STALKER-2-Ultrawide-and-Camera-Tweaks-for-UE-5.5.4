...
namespace
{
    ...
    DWORD WINAPI Initialize(void*)
    {
        ...
        const auto logPath = std::filesystem::path(modulePath).remove_filename() / "STALKER2CinematicPostExitABCOutputDiagnostic204.log";
        std::ofstream(logPath, std::ios::out | std::ios::trunc).close();
        try {
            g_logger = spdlog::basic_logger_mt("STALKER2CinematicPostExitABCOutputDiagnostic204", logPath.string(), true);
