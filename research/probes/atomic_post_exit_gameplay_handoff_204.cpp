...
namespace
{
    ...
    std::atomic<bool> g_atomicHandoffConsumed{false};
    ...
    void RunAtomicGameplayHandoff(SafetyHookContext& context)
    {
        Log("Atomic handoff: invoking existing gameplay replay B at recovery boundary.");
        ReplayManualTransitionOriginal(context);

        float aspect = 0.0f;
        std::uint8_t flags = 0;
        const auto source = static_cast<std::uintptr_t>(context.rsi);
        const bool stateReady = g_state.load(std::memory_order_acquire) == ReplayState::AppliedConstrainPass;
        const bool fieldsReadable = SafeRead(source + kAspectOffset, aspect) &&
            SafeRead(source + kFlagsOffset, flags);
        Log("Atomic handoff: B result stateApplied=", stateReady,
            " fieldsReadable=", fieldsReadable, " aspect=", aspect,
            " flags=0x", std::hex, static_cast<int>(flags), std::dec, ".");

        if (!stateReady || !fieldsReadable) {
            Log("Atomic handoff: C refused because B contract was not established; fail-closed.");
            return;
        }

        Log("Atomic handoff: invoking existing gameplay replay C at recovery boundary.");
        ReplayManualTransitionOriginal(context);
        SafeRead(source + kAspectOffset, aspect);
        SafeRead(source + kFlagsOffset, flags);
        Log("Atomic handoff complete: state=", ReplayStateName(g_state.load(std::memory_order_acquire)),
            " aspect=", aspect, " flags=0x", std::hex, static_cast<int>(flags), std::dec,
            " duplicateReplayGuard=consumed.");
    }
    ...
    {
        g_atomicHandoffConsumed.store(false, std::memory_order_release);
        bool expected = false;
    ...
    void ReplayManualTransition(SafetyHookContext& context)
    {
        ...
        if (state == CoordinatorState::CinematicActive) {
            Log("Global coordinator: gameplay writer observed during CinematicActive; replay suppressed.");
            return;
        }
        if (state == CoordinatorState::CinematicExiting) {
            ...
            if (flags == 0x04 && std::isfinite(currentFov) && std::isfinite(targetFov) && delta <= kRecoveryEpsilon) {
                ...
                if (!g_atomicHandoffConsumed.exchange(true, std::memory_order_acq_rel)) {
                    Log("Global coordinator: native recovery complete; entering atomic gameplay handoff.");
                    RunAtomicGameplayHandoff(context);
                    return;
                }
                ...
                Log("Global coordinator: native recovery observed; replay suppressed. current=",
                    currentFov, " target=", targetFov, " delta=", delta, " flags=0x",
    ...
    DWORD WINAPI Initialize(void*)
    {
        ...
        const auto logPath = std::filesystem::path(modulePath).remove_filename() / "STALKER2CinematicGlobalCoordinatorIntegration204.log";
        std::ofstream(logPath, std::ios::out | std::ios::trunc).close();
        try {
            g_logger = spdlog::basic_logger_mt("STALKER2CinematicGlobalCoordinatorIntegration204", logPath.string(), true);
